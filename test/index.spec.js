...
const mockWordsData = {
	metadata: {
		...
		totalCategories: 5,
		categories: {
			gre: { name: 'GRE', fullName: 'Graduate Record Examination', totalWords: 30 },
			toefl: { name: 'TOEFL', fullName: 'Test of English as a Foreign Language', totalWords: 25 },
			ielts: { name: 'IELTS', fullName: 'International English Language Testing System', totalWords: 25 },
			gmat: { name: 'GMAT', fullName: 'Graduate Management Admission Test', totalWords: 10 },
			sat: { name: 'SAT', fullName: 'Scholastic Assessment Test', totalWords: 10 }
		}
	},
	words: {
		gre: [
...
describe('Answerbook API Tests', () => {
	...
	beforeAll(() => {
		// 設置 mock KV 數據
		if (env.ANSWERS_BOOK) {
			env.ANSWERS_BOOK.get = async (key) => {
				if (key === 'words_processed') {
					return JSON.stringify(mockWordsData);
				}
				return null;
			};
		}
